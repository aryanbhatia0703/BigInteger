...
struct BigInteger  sub2(struct BigInteger b1,struct BigInteger b2)
{
    struct BigInteger res;
    res.head=NULL;
    int b=0,diff;
...
struct BigInteger  div1(struct BigInteger b1,struct BigInteger b2)
{
    struct BigInteger res;
    res.head=NULL;
    struct BigInteger res1=initialize("0"); 
    struct BigInteger b2Copy = b2;
    int comparison;

 
    comparison = compare(b1, b2);
    ...
    while(compare(res,b2)!=-1)
        {
            res=sub(b2,res);
            res1=modadd(res1,initialize("1"));
